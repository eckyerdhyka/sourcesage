...
def profile(request,token):
    ...
    if not TokenGenerator.check_token(token):
        return generate_response(
    ...
    input_data["age"] = user.age
    input_data["gender"] = user.gender
    return generate_response(
            data=input_data, message="User profile", status=HTTP_201_CREATED
    )
